...
- **Kafka producer** — publishes `AuditLogEvent` to `emergency-access-events` on every emergency access **cache miss**, via `kafkaTemplate.send(topic, event)` with no explicit key — the topic runs a single partition (see below), so ordering isn't currently a partition-assignment concern, but if partition count ever increases this would need a key (e.g. `userId`) added to preserve per-user ordering (Day 5)
...
↓
kafkaTemplate.send(KafkaTopics.AUDIT_EVENTS, event)   ← fire and forget, no explicit key (Day 5)
↓
